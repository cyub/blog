...
寻址方式 | 寻址指令 | 解释
---|---|---
立即寻址 | movl $number, %eax | 将number直接加载到寄存器或存储位置
直接寻址 | movl 0x123, %eax | 将内存地址0x123加载到%eax
索引寻址， 也称为变址寻址方式 | movl string_start(, %ecx, 1), %eax | 从string_start出开始，将该地址与1 * %ecx相加，并将所得值加载到%eax中
间接寻址方式 | mov (%eax), %ebx | 从寄存器指定的地址加载值
基址寻址方式 | movl 4(%eax), %ebx | 类似于间接寻址方式，但基址寻址方式会将寄存器地址与一个常量值相加 这里面是将%eax+4之后的地址加载到%ebx
