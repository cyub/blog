...
![寄存器](https://static.cyub.vip/images/202007/register.png)
...
每个运行的计算机程序都是用叫做栈的内存区来使函数正常工作。计算机的栈处理内存地址的最顶端，可以通过pushl指令将值压入栈顶。指令popl将值从栈顶弹出。

每当值进行入栈或出栈时候，栈在内存中是向下增长的，栈寄存器%esp总是包含一个指向当前栈顶的指针。当push1数据入栈时，%esp所包含的指针值会减去4，从而指向新的栈顶，同理popl则会使%esp的值增加4
